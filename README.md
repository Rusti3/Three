...
Что сделано:
- Убран туман из сцены.
- Добавлен ползунок в правом верхнем углу для изменения размера поезда.
- Добавлена логика масштабирования поезда через `trainSizing`.
- Добавлены unit-тесты для `trainSizing`.

Зачем эти функции:
- Сцена стала визуально чище без тумана.
- Пользователь может быстро подгонять размер поезда в рантайме.

Что изменилось для пользователя:
- Нет тумана.
- В правом верхнем углу появился ползунок `Train size`.

### v0.2.6 (2026-02-23)

Что сделано:
- Подключена новая модель поезда `699c27994f79ee2ab165166d.glb`.
- Обновлены тесты на ссылку на новую модель.

Зачем эти функции:
- Используется актуальный ассет поезда.

Что изменилось для пользователя:
- В сцене теперь отображается новая модель поезда.
